class MyHTMLRenderer < Redcarpet::Render::HTML
  def header(text, header_level, options)
      ...
      if header_level == 2
        if @h2_is_open == nil
          @h2_is_open = true  
          s = "<div class='h2bracket'><div class='h2item h2box'><h#{header_level}>#{text}</h#{header_level}></div><br><br>"
          return s
        end

        s = "</div>\n<div class='h2bracket'><div class='h2item h2box'><h#{header_level}>#{text}</h#{header_level}></div><br><br>"
        @h2_is_open = true

        return s
      end
      if header_level == 3
        s = "<br><br><div class='h3bracket'><div class='h3item h3box'><br><br><h#{header_level}>#{text}</h#{header_level}></div></div>"
        return s
      end
